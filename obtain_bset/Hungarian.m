% km分配算法
%
% 输入;Perf边赋权完全偶图，即输入每一个agent(受控机)对于每一个target（目标)的代价值
% 输出：Matching：最优分配。Matching(i,j)=1表示将第i个agent分配给第j个target
%                  i:agent编号
%                  j:target编号
% Cost：整体最小代价
% Weight：Weight(i,j)表示各agent针对每个target的代价
%          i:agent编号
%          j:target编号
function [Matching,Cost,Weight] = Hungarian(Perf)
% 
...
%
%WEIGHT return the initially cost matrix
