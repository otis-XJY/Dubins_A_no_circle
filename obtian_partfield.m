function [total_field,boundary]=obtian_partfield(map_size,End_Point,outline_all,attractive_force,repulsive_force,center)
...
x_polygon = [center(1)-map_size/2, center(1)+map_size/2, center(1)+map_size/2, center(1)-map_size/2];
y_polygon = [center(2)-map_size/2,center(2)-map_size/2, center(2)+map_size/2, center(2)+map_size/2];

boundary=[x_polygon;y_polygon];
% 定义起点、终点和障碍物位置
...
is_inside = inpolygon(outline_all(:,1), outline_all(:,2), x_polygon, y_polygon);
id=find(is_inside==1);
if ~isempty(outline_all)
    obstacles=outline_all(id(1:10:end),1:2);
end



[X, Y] = meshgrid(center(1)-map_size/2:resolution:center(1)+map_size/2, center(2)-map_size/2:resolution:center(2)+map_size/2);
