 function [param_all,param_best] = DubinsAvoid(Start_Point,End_Point,Avoid_Center,r_s,r_e,R,Stepsize,check)
...
%% 挑选出最短路径
    ...
    for j=1:length(check)
        for i = 1:length(path1{check(j)}(1,:))
            Distance1(i) = sqrt((path1{check(j)}(1,i)-Dx)^2+(path1{check(j)}(2,i)-Dy)^2);
        end
        
        for i = 1:length(path2{check(j)}(1,:))
            Distance2(i) = sqrt((path2{check(j)}(1,i)-Dx)^2+(path2{check(j)}(2,i)-Dy)^2);
        end
        
        for i = 1:length(path3{check(j)}(1,:))
            Distance3(i) = sqrt((path3{check(j)}(1,i)-Dx)^2+(path3{check(j)}(2,i)-Dy)^2);
        end
        
        for i = 1:length(path4{check(j)}(1,:))
            Distance4(i) = sqrt((path4{check(j)}(1,i)-Dx)^2+(path4{check(j)}(2,i)-Dy)^2);
        end
    end
...
%     display([index1,index2,index3,index4,indexA])
    error = 0.0001;

    %% 选择最优
    if mDistance(index1) >= param_best.R - error%最短的是否不过障碍圆
        param_best = paramcell{index1};
    elseif mDistance(index2) >= param_best.R - error && Dubins_Length1(index2)<= min(Dubins_Length2)
        param_best = paramcell{index2}; 
    elseif  Dubins_Length1(index2) > min(Dubins_Length2)
        param_best = paramcell{indexA+4};
    elseif  mDistance(index3) >= param_best.R - error && Dubins_Length1(index3)<= min(Dubins_Length2)
        param_best = paramcell{index3}; 
    elseif  Dubins_Length1(index3) > min(Dubins_Length2)
        param_best = paramcell{indexA+4};
    elseif  mDistance(index4) >= param_best.R - error && Dubins_Length1(index4)<= min(Dubins_Length2)
        param_best = paramcell{index4}; 
    elseif  mDistance(index4) < param_best.R || Dubins_Length1(index4) > min(Dubins_Length2)
        param_best = paramcell{indexA+4};
    end  
    
%     param_best.vtheta = [Start_Theta,End_Theta];
%     param_best.Length = sum(param_best.length);   
