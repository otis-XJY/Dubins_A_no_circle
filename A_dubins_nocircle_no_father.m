function [final_path,open,close]=A_dubins_nocircle_no_father(Start_Point,End_Point,obs,sure,r,obs_no_circle,obs_no_circle_in,outline_all,Stepsize,total_field)
...
%%
    while 1
        if times>=1
            ...
            [open_f,open,pos_id_mayday]=update_open_mayday_father(Start_Point,End_Point,outline_all,r,Stepsize,obs_no_circle,total_field,close,open,open_f,pos_id_mayday);                  
            % 将数组从小到大排列并记录索引
            [sorted_A, idx] = sort(open_f);
...
%                     min_indices(mm)
                ...
                times=length(find(pos_id_mayday==close{end}.pos_id));
    %             [open_f,open,pos_id_mayday]=update_open_mayday(Start_Point,End_Point,outline_all,r,Stepsize,obs_no_circle,total_field,close,open,open_f,pos_id_mayday);   
