function [final_path,open,close]=A_dubins_nocircle(Start_Point,End_Point,obs,sure,r,obs_no_circle,obs_no_circle_in,outline_all,Stepsize,total_field)
...
while isempty(open_f)||all(isnan(open_f)==1)
    display('open is empty!Mayday!even in begin!!');

%     close=close(1:end-1);
    param_all={};
    for i=1:length(obs_no_circle)
        if ~isempty(obs_no_circle{i})
%         [param_all{i},param_best{i}] = Dubins_obs(Start_Point,End_Point,obs(i,1:2),r,r,obs(i,3),Stepsize,i,-1);
        [param_all{1,i},param_best{1,i}] = Dubins_obs_nocircle(Start_Point,End_Point,outline_all,r,r,r,Stepsize,i,-1,obs_no_circle,total_field);
%         [param_all{1,i},param_best{1,i}] = Dubins_obs([close{end}.point(:,end)',close{end}.vtheta],End_Point,obs(i,1:2),r,r,obs(i,3),Stepsize,i,close{end}.pos_id);
%         [param_all{2,i},param_best{2,i}] = Dubins_obs([close{end}.point(:,end)',close{end}.vtheta],End_Point,obs(i,1:2),close{end}.R,r,obs(i,3),Stepsize,i,close{end}.pos_id);
        end
    end
...
%     find(pos_id_mayday==close{end}.pos_id)
%         open_f
    times=length(find(pos_id_mayday==close{end}.pos_id));
    if times>1
        display(['add open to close more',num2str(times-1)])

        
        % 将数组从小到大排列并记录索引
        [sorted_A, idx] = sort(open_f);
        
        % 取最小的两个数及其对应的索引
        min_values = sorted_A(1:times-1);
        min_indices = idx(1:times-1);
        for mm=1:length(min_indices)
            close{end+1}=open{min_indices(mm)};                        
            open(min_indices(mm))=[];
            open_f(min_indices(mm))=[];
        end
...
% param_safe=struct();
% param_safe.r=obtain_safeparam(path_safe_flag.r,param_all(1,:));
% param_safe.R=obtain_safeparam(path_safe_flag.R,param_all(2,:));
% 
% safe_obs_num=max(length(param_safe.r),length(param_safe.R));
...
%%
    if times>=1
        %%
        display(['add open to close more',num2str(times)]);
    ...
    %     [open_f,open]=update_open(path_safe_flag.R,param_all(2,:),End_Point,open_f,open,close);
        ...
        pos_id_mayday=[pos_id_mayday,-1];
            ...
            obs_to_avoid=obtain_obs_to_avoid(close{end}.point(:,end)',End_Point,outline_all);
            ...
            for i=1:length(obs_to_avoid)
                if ~isempty(obs_no_circle{obs_to_avoid(i)}) && obs_to_avoid(i)~=close{end}.pos_id
                    [param_all{1,i},param_best{1,i}] = Dubins_obs_nocircle([close{end}.point(:,end)',close{end}.vtheta],End_Point,outline_all,r,r,r,Stepsize,obs_to_avoid(i),close{end}.pos_id,obs_no_circle,total_field);
    %                 [param_all{2,i},param_best{2,i}] = Dubins_obs_nocircle([close{end}.point(:,end)',close{end}.vtheta],End_Point,outline_all,close{end}.R,r,sure,Stepsize,i,close{end}.pos_id,obs_no_circle);
                end
            end
    ...
    %          [flag_all,obs_id_all]=obtain_flag_safe(2,param_all,obs,Start_Point,End_Point,obs_know);
        ...
        safe_obs_num=max(length(param_safe.r),length(param_safe.R));
        pos_id_mayday=[pos_id_mayday,close{end}.pos_id];
    %         param_safe=struct();
    %         param_safe.r=obtain_safeparam(path_safe_flag.r,param_all(1,:));
    %         param_safe.R=obtain_safeparam(path_safe_flag.R,param_all(2,:));
    %         
    %         safe_obs_num=max(length(param_safe.r),length(param_safe.R));
        ...
        % 将数组从小到大排列并记录索引
        [sorted_A, idx] = sort(open_f);
        if length(open_f)>=times
            %%
            % 取最小的两个数及其对应的索引
            min_values = sorted_A(1:times);
            min_indices = idx(1:times);
            for mm=1:length(min_indices)
                close{end+1}=open{min_indices(mm)};                        
                open(min_indices(mm))=[];
                open_f(min_indices(mm))=[];
...
%     while find(pos_id_mayday==close{end}.pos_id)
%         display('试图死锁，但我不给机会')
%         close=close(1:end-1);
% 
%         if isempty(close)
